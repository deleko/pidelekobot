...
def gas_search(user_lat, user_lon):
    ...
    for i in range(len(gas_list)):
        ...
        if filtered:
            km_distance = round(geodistance(user_lat, user_lon, gaslat, gaslon), 2)
    ...
    gas_found = sorted(gas_found, key=lambda x: x['km_distance'])
    gas_found = gas_found[0:5]

    msg_list = [f"⛽ <b>{gas['title']}</b> \n"
                f"- Diesel: {gas['diesel_price']}€ \n"
                f"- Gasolina95: {gas['gasolina95_price']}€ \n"
                f"📍 <a href='https://maps.google.com/maps?q={gas['lat']},{gas['lon']}'>Google Maps</a> {gas['km_distance']} Km \n\n"
